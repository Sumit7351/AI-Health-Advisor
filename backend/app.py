...
def predict():
    ...
    for sym in selected_symptoms:
        # Normalize: convert spaces to underscores to match dataset columns
        normalized_sym = sym.replace(" ", "_")
        if normalized_sym in input_dict:
            input_dict[normalized_sym] = 1
        else:
             print(f"Warning: Symptom '{sym}' (normalized: '{normalized_sym}') not found in dataset columns.")
